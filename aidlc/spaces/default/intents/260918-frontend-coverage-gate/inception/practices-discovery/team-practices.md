# Prácticas del equipo — intent `260918-frontend-coverage-gate`

> Prácticas afirmadas por el lead de pipeline/deploy para la etapa Practices Discovery
> (RE-RUN), integrando la entrevista humana (Q1–Q8) y las tres contribuciones ciegas
> (quality/developer/devsecops). Parte de la línea base afirmada en `team.md` y la
> adapta a ESTE intent (cobertura de tests del frontend Angular + significatividad del
> gate del pipeline: FR10 y FR17.1). Solo se ajusta lo que este intent cambia; el resto
> se arrastra tal cual. La promoción a `memory/{team,project}.md` la hace el promote
> tool en el gate.
